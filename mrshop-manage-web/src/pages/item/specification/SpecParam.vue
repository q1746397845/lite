<template>
  <div>
    <v-data-table :headers="headers" :items="params" hide-actions class="elevation-0">
      ...
      <template slot="no-data">该分组下没有参数</template>
    </v-data-table>
  ...
  methods: {
    ...
    deleteParam(id) {
      this.$message.confirm("确认要删除该参数吗？").then(() => {
        this.$http
          .delete("specParam/deleteSpecParam?id=" + id)
          .then(resp => {
            if(resp.data.code == 200){
